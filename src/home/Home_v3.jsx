...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // === Global phase-driven deformation (no radial sweep) ===
    const PHASE_SPEED = 0.4;      // radians per second: shifts f(θ+φ) smoothly
    const AMP_MOD = 0.15;         // subtle global amplitude modulation
    ...
    function draw(tsMs) {
      ...
      const phi = tSec * PHASE_SPEED; // global phase drift (radians)
      const fRaw = new Float32Array(N);
      for (let i = 0; i < N; i++) {
        const th = angles[i] + phi;
        let sum = 0;
        for (let m = 1; m <= M; m++) sum += a[m] * Math.cos(m * th) + b[m] * Math.sin(m * th);
        const CLAMP = 0.7;
        fRaw[i] = Math.max(-CLAMP, Math.min(CLAMP, sum));
      }
      const f = gaussianSmooth(fRaw, 9); // stronger angular smoothing
      ...
      // === Render rings without sweep (uniform density), very smooth ===
      const amp = 1 + AMP_MOD * Math.sin(tSec * 0.2); // slow overall breathing
      for (let ring = 0; ring < TOTAL; ring++) {
        const rPow = Math.pow(ring / (TOTAL - 1), OUTER_GAIN_EXP);
        ctx.globalAlpha = 1;
        ctx.lineWidth = 0.75 + rPow * 0.55;
        ctx.miterLimit = 1.2;

        // fixed high-quality resampling for stability
        const baseCount = Math.max(220, Math.floor(angles.length * 1.4));
        const smoothPts = resampleClosed(pts[ring], baseCount);

        // apply small amp modulation to radius by scaling points from origin
        for (let i = 0; i < smoothPts.length; i++) {
          const x = smoothPts[i][0];
          const y = smoothPts[i][1];
          smoothPts[i] = [x * amp, y * amp];
        }
