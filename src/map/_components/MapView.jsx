...
const MapView = ({
    ...
    const loadStaticData = async () => {
      try {
        const [realLocationsRes, exhibitionsRes, coursesRes] = await Promise.all([
          fetch("/map/real_location.json"),
          fetch("/map/exhibition.json"),
          fetch("/map/courses.json"),
        ])
  ...
  useEffect(() => {
    ...
    if (!shortestPaths?.features?.length || !staticData.realLocations.length) return

    const locatIds = shortestPaths.features.map((path) => path.properties.end_name)

    if (locatIds.length === 0) return

    console.log(`[v0] Processing locations for IDs: ${locatIds.join(",")}, with type: ${selectedType}`)
      ...
      try {
        ...
        const processedLocations = filteredRealLocations.map((location) => {
          ...
          if (selectedType === "exhibition") {
            ...
            const locationCourses = staticData.courses.filter((course) => course.locat_id === location.locat_id)
